...
//use crate::{arithmetic::FieldElement, NonZeroScalar, Scalar, AffinePoint, ProjectivePoint};
use crate::{arithmetic::FieldElement, AffinePoint, FieldBytes, NonZeroScalar, Scalar};
use core::{cmp, fmt};
use elliptic_curve::{
    bigint::U256,
    ops::Reduce,};
...
impl pre_Signature {
    /// Size of a Taproot sm2 signature in bytes.
    ...
    pub fn pub_from_bytes(bytes:&[u8]) -> Result<Self> {
        bytes.try_into()
    }
...
mod tests {
    ...
    const BIP340_SIGN_VECTORS: &[SignVector] = &[
        SignVector {
            ...
            //Y: hex!("F9308A019258C31049344F85F89D5229B531C845836F99B08601F113BCE036F9"),
        },
        SignVector {
            index: 1,
    ...
    fn bip340_sign_vectors() {
        for vector in BIP340_SIGN_VECTORS {
            ...
            let sig = sk
                .try_pre_sign_prehashed(&vector.message, &vector.aux_rand, &hex!("25D1DFF95105F5253C4022F628A996AD3A0D95FBF21D468A1B33F8C160D8F517"),)
                .unwrap_or_else(|_| {
                    panic!(
                        "low-level sm2 signing failure for index {}",
                        vector.index
                    )
                });
    ...
    #[test]
    fn pre_to_sign() {
        let secret_key = hex!("0B432B2677937381AEF05BB02A66ECD012773062CF3FA2549E44F58ED2401710");
        let sk = SigningKey::from_bytes(&secret_key).unwrap();
        let public_key = hex!("25D1DFF95105F5253C4022F628A996AD3A0D95FBF21D468A1B33F8C160D8F517");
        assert_eq!(sk.verifying_key().to_bytes().as_slice(), &public_key);
        //let message = hex!("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
        let test_signature = hex!(
        ...
        let sig = sk
            .try_sign_prehashed(&test_signature, &y)
                .unwrap_or_else(|_| {
                    panic!(
                        "low-level sm2 signing failure for index 1"   
                    )
                });
